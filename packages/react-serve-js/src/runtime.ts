...
export function useRoute() {
  ...
  if (!context) throw new Error('useRoute must be used inside a Route');
  return context;
}
...
export function useSetContext(key: string, value: any) {
  const context = routeContext.getStore();
  if (context) {
    context.middlewareContext.set(key, value);
  } else {
...
export function useContext(key: string) {
  const context = routeContext.getStore();
  if (context) {
    const routeValue = context.middlewareContext.get(key);
...
export function serve(element: ReactNode) {
  ...
  const createExpressHandler = (
    ...
    const wrapped: RequestHandler = async (
      ...
      return routeContext.run(context, async () => {
        try {
